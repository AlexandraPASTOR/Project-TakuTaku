...
describe("Installation", () => {
    test("You have created /client/src/App.tsx", async () => {
        expect(node_fs_1.default.existsSync(`${__dirname}/../../client/src/App.tsx`)).toBe(true);
    });
});
const checkEmailExists_1 = require("../src/middleware/checkEmailExists");
const authRepository_1 = __importDefault(require("../src/modules/auth/authRepository"));
...
const mockedAuthRepo = authRepository_1.default; //je veux accéder à toute les fonctions du module userRepo, comme des fonctions Jest mockées(simulées)
describe("checkEmailExists", () => {
    it("doit renvoyer 409 si l'email existe déjà", async () => {
        ...
        mockedAuthRepo.findByEmail.mockResolvedValue({
            id: 1,
            mail: "test@test.com",
        }); // quand j'appelle la fonction findByEmail je retourne un user factice
        // 2. On crée les mocks de req, res, next
        const req = {
            body: { mail: "test@test.com" },
        }; //permet de ne pas créer toutes les autres propriétés du vrai Request
        const res = {
            status: jest.fn().mockReturnThis(),
            send: jest.fn(),
        }; //simule une réponse
        const next = jest.fn(); //simule une fonction next
        // 3. On appelle le middleware
        await (0, checkEmailExists_1.checkEmailExists)(req, res, next);
...
describe("hachage de mot de passe", () => {
    ...
    it("permet de vérifier le mot de passe via bcrypt.compareSync", () => {
        const hashPassword = bcryptjs_1.default.hashSync(password, 8);
        const isPasswordValid = bcryptjs_1.default.compareSync(password, hashPassword);
        expect(isPasswordValid).toBe(true);
    });
    it("échoue si le mot de passe est incorrect", () => {
        const hashPassword = bcryptjs_1.default.hashSync(password, 8);
        const isPasswordValid = bcryptjs_1.default.compareSync("MauvaisMotDePasse", hashPassword);
