...
function Note() {
  ...
  // Pour la suppression d'une note
  const deleteNote = async () => {
    if (!animeSelected || !user) return;
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/note/${animeSelected.id}/${user.id}`,
        {
  ...
  // Au clic sur une étoile, appel la fonction en question create/update/delete
  const toggleRating = (star: number) => {
    if (!user) {
      toast.error("Vous devez être connecté pour noter un animé");
      return;
    }
    if (userRated !== null && userRated !== undefined) {
      if (userRated === star) {
        deleteNote();
        toast.success("Votre note a bien été supprimée");
        return;
      } else {
        updateNote(star);
  ...
  return (
    <section className="flex gap-2 w-50">
      <section className="flex space-x-0.5 mb-1">
        {[1, 2, 3, 4, 5].map((star) => {
          // Si hoveredRating n’est pas null, utilise-le. Sinon, utilise userRated. Si l'utilisateur n'est pas connecté, activeRating sera null
          const activeRating = user ? (hoveredRating ?? userRated) : null;
          return (
            <button
              type="button"
              key={star}
